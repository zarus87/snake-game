let field = document.createElement("div"); //создаём новый div
document.body.appendChild(field); // добавляем этот div в body
field.classList.add("field"); //добавляем divу class field

for (let i = 1; i < 101; i++) {
  let excel = document.createElement("div"); //coздаём новый div
  field.appendChild(excel); // помещаем все divы внутрь класса field
  excel.classList.add("excel"); // присваимаем класс excel каждому блоку
}

let excel = document.getElementsByClassName("excel"); // находим все элементы excel
let x = 1,
  y = 10;
